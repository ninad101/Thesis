function [edbn, opts] = edbnsetup(edbn, opts)
    ...
    if ~isfield(opts,'momentum'),  opts.momentum  = 0.005; end
    if ~isfield(opts,'temp'),      opts.temp      =  0.005; end
    if ~isfield(opts,'tau_m'),     opts.tau_m     =    5; end %5
    if ~isfield(opts,'tau_s'),     opts.tau_s     =  0.001; end
    if ~isfield(opts,'t_ref'),     opts.t_ref     =  0.002; end
    if ~isfield(opts,'v_thr'),     opts.v_thr     =  0.35; end
    ...
    if ~isfield(opts,'distance'),  opts.distance  = 104.56433185; end % value for mnist 
